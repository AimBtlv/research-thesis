## 2. How was Created allograph_all_v6?
...
**Note** The unmatched remainder is fully accounted for by rows with no usable `unicode_id`: rows carry the undeciphered-sign placeholder `X`*, and  rows are `not_identified` and have no `unicode_id` to look up in the first place.
