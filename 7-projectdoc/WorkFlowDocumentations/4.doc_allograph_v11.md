...
| `inherited_from_parent` | Row has no code of its own, `unicode_id` was inherited from the parent `@sign`'s own code ( an archaic catalogue form such as `LAK797` inheriting the code of its modern equivalent sign `A`). Kept out of `not_identified` deliberately, since the row *does* carry a usable Unicode value, just not one declared at its own level. |
